...
"读取TXT文件中迭代的state数据==================================================="
statefilename="./history/priorstate_test_s5.txt"#存储的先验数据的state列表txt文件
...
ttt=valuegmmgamer.valueUCBhelper_alpha(memoryset.probmemoryunit,kappa=3,iternum=iteration_t,count=0)
tu=ttt.tolist()
listaaa.append(tu)
with open('QP_HPP.txt','a') as f:#记录每次AF选点的参数
...
#    distriubuteculsterdata=distriubuteculsterdata.append(newdataset)
    ...
    ttt=valuegmmgamer.valueUCBhelper_alpha(memoryset.probmemoryunit,kappa=3,iternum=iteration_t,count=simucount)
    tu=ttt.tolist()
    listaaa.append(tu)
